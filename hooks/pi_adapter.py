...
## 平台
PI / Perplexity（Agent 形态）。

## Hook 文档
- ⚠️ **未找到公开官方 hook 文档**（已检索，不编造链接）。
  因此下文的「事件名 / stdin 字段 / stdout 格式」均为本项目通用约定，**属占位实现**，待官方文档出现后按 §stdin 字段映射表 对齐。

## 核实状态：⚠️ 未核实
- Perplexity / PI 目前**未公开** Agent hook 接口文档，因此本文件的
  事件名 / stdin 字段 / stdout 格式均按本项目通用约定实现，**属占位**。
- 不编造官方链接。若官方文档出现，只需改本文件 `emit()` 一处。

## 与已验证平台的差异（当前假设）
- 配置位置：**假设**为 `~/.pi/hooks.json`（顶层 `hooks` 键）。
  `templates/pi-hooks.json` 是占位结构。
